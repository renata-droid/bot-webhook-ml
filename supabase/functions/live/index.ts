...
// Carteira aberta de hoje. Só interessa negócio com "Vendedor" preenchido —
// descartando o resto aqui, o que sobra é a carteira do comercial, não a conta inteira.
function abertos() {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (truncou.size) avisos.push(
      `A varredura de negócios <b>${[...truncou].join(" e ")}</b> bateu no teto de páginas — ` +
      `pode faltar negócio antigo. Avisa que eu aumento o limite.`);
