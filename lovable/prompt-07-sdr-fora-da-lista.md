...
mensagem, e ajuste o bloco de avisos como descrito abaixo. Isto substitui o
prompt anterior sobre a `foraDaLista` — se ele ainda não foi aplicado, ignore,
já está tudo neste arquivo.

## O que mudou na lógica

Agora são dois cortes, e a página usa cada um no seu lugar:

- **Os cards do topo** (Conectados, SQL, OPS, SAL e as taxas) contam **todo
  mundo** — `funilSdr` já faz isso sozinho, a página não muda. É o que tem que
  bater com o Pipedrive.
- **A tabela "Por SDR", os gráficos por nota e a matriz de requalificação**
  continuam mostrando só o time — `porSdr`, `porNota` e `requalificacao` já
  cortam sozinhos.

A `tecnologia@awsales.io` passou a contar como SDR (é a IA que conecta), então
ela volta a aparecer nos gráficos e na tabela.

## O bloco de avisos

Chame `foraDaLista(rows, filtro)`. Ele devolve
`{ semCampo: {conectados, sql, ops, sal}, outroNome: {…}, nomes: [{nome, conectados, sql, ops, sal}] }`.

Troque o texto dos avisos por, no máximo, estas duas linhas (a linha some
quando o número é zero):

- `Dos {conectados do card}, {semCampo.conectados} estão sem o campo "Vendedor SDR"`
- `e {outroNome.conectados} com nome fora do time: {os 3 primeiros de nomes}{ e mais N}`

E, embaixo da tabela "Por SDR", uma linha em fonte menor explicando a diferença:

`as barras somam {soma da coluna conectados} — os outros {semCampo.conectados + outroNome.conectados} não têm SDR do time no negócio`

Os avisos estão saindo com as tags `<b>` visíveis no meio do texto. Tire as
tags da string; o negrito faz por CSS.
